...
def plot_show(fuse_df, fsspec_df):
    ...
    for i, block_size in enumerate(block_sizes):
        for j, numjob in enumerate(num_jobs):
            ...
            fuse_filtered = fuse_df[(fuse_df['bs'] == block_size) & (fuse_df['numjobs'] == numjob)].copy()
            fsspec_filtered = fsspec_df[(fsspec_df['bs'] == block_size) & (fsspec_df['numjobs'] == numjob)].copy()

            fuse_filtered["bw_numeric"] = fuse_filtered["bw"].apply(convert_to_numeric)
            fsspec_filtered["bw_numeric"] = fsspec_filtered["bw"].apply(convert_to_numeric)

            # Aggregate data to ensure only one row per file size (average bandwidth)
            fuse_filtered = fuse_filtered.groupby('file_size').agg({'bw_numeric': 'mean'}).reset_index()
            fsspec_filtered = fsspec_filtered.groupby('file_size').agg({'bw_numeric': 'mean'}).reset_index()

            # Ensure the data aligns with the file_sizes order
            fuse_filtered = fuse_filtered.set_index('file_size').reindex(file_sizes).reset_index()
            fsspec_filtered = fsspec_filtered.set_index('file_size').reindex(file_sizes).reset_index()

            # Plot the Fuse bandwidth bars for each file size
            ax.bar(
                np.arange(len(file_sizes)) - 0.2,
                fuse_filtered['bw_numeric'],
                0.4, label=f'Fuse {block_size}', color='blue'
            )
            ...
            ax.bar(
                ...
                0.4, label=f'FSSpec {block_size}', color='orange'
            )
