...
def parse_fsspec_log(file_path):
    ...
    data = {
        "rw": "read" if file_templete_name.split('_')[1] == "download" else "write",
        "bw": convert_to_MB(log_data["metrics"].get("bytes_per_second", 0)),
        "iops": process_iops(file_templete_name, log_data["metrics"].get("ops_per_second", 0)),
        "file_size": file_templete_name.split('_')[3],
        "bs": file_templete_name.split('_')[4],
...
def summarize_results(results):
    ...
    df_grouped['bw'] = df_grouped['bw_numeric'].apply(lambda x: str(round(x, 1)) + "MiB/s")
...
def main():
    ...
    parser.add_argument("--inputs_dir", required=True, help="Directory containing FSSPEC log files")
    parser.add_argument("--outputs_dir", required=True, help="Directory to save output CSV files")
