...
def main(args):
    ...
    print(f"Total bytes read: {humanfriendly.format_size(metrics['total_bytes'], binary=True)}")
    ...
    if metrics['read_time'] > 0:
        ...
        print(f"Throughput: {humanfriendly.format_size(throughput, binary=True)} per second")
