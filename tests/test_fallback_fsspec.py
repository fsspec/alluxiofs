...
from alluxiofs import FallbackFileSystem
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Benchmarking script for reading files with FallbackFileSystem"
    )
    parser.add_argument(
        "--dataset",
        type=str,
...
def process_path(fallback, path, metrics):
    if path["type"] == "file":
        metrics["total_files"] += 1

        file_read_start = time.time()
        with fallback.open(path["name"], "rb") as f:
            data = f.read()
        ...
        contents = fallback.ls(path["name"], detail=True)
        for item in contents:
            process_path(fallback, item, metrics)

def main(args):
    start_time = time.time()
    fsspec.register_implementation("fallback", FallbackFileSystem, clobber=False)
    fallback = fsspec.filesystem("fallback", target_protocol="s3")
    ...
    initial_path_contents = fallback.ls(args.dataset, detail=True)
    for path in initial_path_contents:
        process_path(fallback, path, metrics)
